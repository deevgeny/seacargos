...
def setup_db(app):
    ...
    cur = conn.seacargos.users.find_one({"name": app.config["ADMIN_NAME"]})
    if cur is None:
        pwd = generate_password_hash(app.config['ADMIN_PASSWORD'])
        conn.seacargos.users.insert_one(
            {'name': app.config['ADMIN_NAME'],
            'password': pwd,
            'role': 'admin'}
        )
    cur = conn.seacargos.users.find_one({"name": app.config["USER_NAME"]})
    if cur is None:
